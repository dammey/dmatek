...
2. **Keep every `data-*` attribute.** The engine finds elements by them. Contract:
   ...
   sequence · `data-rv="rise|pill|up|mask|deal"` scroll reveals · `data-adire` footer strip.
...
- [ ] Adire footer strip under the Contact section (full width, 22px, 2px gold rule below; same tile as D’Matek — see README › Adire bands)
