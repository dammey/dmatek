...
import { z } from "zod";
import { requireStaff, userIdFromBearer } from "../../auth/middleware.js";
 ...
 * records the invited email so this only claims the matching invite. */
adminStaffRouter.post("/link", async (req, res) => {
  ...
  const { data: staff } = await db.from("staff").select("id, active").eq("invite_email", email).is("auth_user_id", null).maybeSingle();
  if (!staff) return res.status(404).json({ error: "No unclaimed staff invite for this email" });
...
adminStaffRouter.post("/", requireStaff("Staff and roles"), async (req, res) => {
  const body = z.object({ name: z.string(), role: z.enum(["Owner", "Sales", "Warehouse", "Engineer", "Support"]), inviteEmail: z.string().email() }).parse(req.body);
  const { data, error } = await db.from("staff").insert({ name: body.name, role: body.role, invite_email: body.inviteEmail, active: true }).select().single();
