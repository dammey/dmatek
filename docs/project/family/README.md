# Handoff: D’Matek family — D’Matek site v3.4 · D’Foundry · D’Source v7.4 · D’Source Admin v2

## Overview
Four products, one bundle:

| # | Product | Domain | Design reference | Spec |
|---|---|---|---|---|
| 1 | D’Matek corporate site | dmatek.ng | `Dmatek Website v3.4.dc.html` | Part 1 |
| 2 | D’Foundry site + Droplet case study | dfoundry.dmatek.ng | `DFoundry Hero.dc.html`, `Droplet Case Study.dc.html` | Part 2 + `DFOUNDRY_PORT.md` |
| 3 | D’Source storefront (D’Emporium for home, D’Provision for business) | dsource.dmatek.ng | `DSource v7.4.dc.html` | Part 3 + `DSOURCE_PORT.md` |
| 4 | D’Source Admin (back office) | admin.dsource.dmatek.ng (internal, auth required) | `DSource Admin v2.dc.html` | Part 4 + `DSOURCE_PORT.md` |

This bundle supersedes `design_handoff_dmatek_v3_4`. Parts 1 and 2 are carried over unchanged except where noted
(D’Foundry now has the Adire footer strip). Parts 3 and 4 are new.

### Build order
1. One monorepo (recommended: Next.js App Router + TypeScript, pnpm workspaces). Shared package `@dmatek/brand`: fonts, colour tokens, `AdireBand`, `PilotLabel`.
2. D’Matek site (Part 1).
3. D’Foundry (Part 2) — exact port.
4. D’Source storefront (Part 3) with a real catalogue/cart/order API.
5. D’Source Admin (Part 4) on the same API, behind staff auth and role permissions.
6. Cross-links (table below). Motion last, all behind `prefers-reduced-motion`.
...
## Shared family components (build once, use in all four)
- **AdireBand** — tile and variants in Part 1 › “Adire bands”. Used: D’Matek (specialists, page strip, footer), D’Foundry (footer), D’Source (footer, 22px, behind prop `adire`, default on).
- **PilotLabel** — Part 1 › “Pilot label”. D’Source uses it for the Device care plan pilot with the standard pilot sentence.
- **Tokens** — deep green `#06382E`, cream `#F5F1E8`, gold `#D4A637`, mid green `#28705A`, body `#3A4A44`, card cream `#EFEADC`. Manrope (D’Matek, D’Source, Admin); D’Foundry has its own fonts.
...
### Added in this release — Adire footer strip
D’Foundry now carries the D’Matek family marker at the very bottom of the page, after the Contact section (which holds the footer line).
- Element: `<div aria-hidden="true" data-adire>`, full width, `height:22px`, background `#06382E` + the Adire tile data-URI (same tile as Part 1 › Adire bands), `background-size:auto 22px; background-repeat:repeat-x`, `border-bottom:2px solid #D4A637`. No top rule.
- Reuse the shared `AdireBand variant="footer"` component from the D’Matek build.

### Still not designed (do not build)
Headline visible on load; tighten the empty green area; gold cable marker; sharper product frames; clearer sound control; phone interaction pass.
...
**Reference:** `DSource Admin v2.dc.html`; split source in `dsource-source/admin-*`. Internal tool, desktop-first (usable from 1024px), Manrope, sample data throughout (header shows a dashed **SAMPLE DATA** tag — remove in production).
...
## Visual reference screenshots (`screenshots/`)
Static captures at ~1190px wide, for orientation only. **The runnable `.dc.html` files are the source of truth** — motion, hover, scroll-driven states and mobile layouts must be checked against them (Playwright loop in the port guides). Scroll reveals were forced visible for capture; some hero text that animates in may appear blank.
- `screenshots/dmatek/` — 01 home · 02 solutions · 03 environment (hospitality) · 04 businesses · 05 work · 06 about · 07 insights · 08 contact
- `screenshots/dfoundry/` — 01 hero (2a Pieces) · 02 problem → fix · 03 services · 04 process · 05 recent work · 06 products · 07 contact (title sequence 2c is scroll-driven: see the file)
- `screenshots/storefront/` — 01 front · 02 D’Emporium · 03 D’Provision · 04 category · 05 product (business) · 06 quote basket drawer · 07 quote flow · 08 flow done · 09 kit overlay · 10 product (home) · 11–12 checkout delivery/payment · 13 track · 14–15 account signed out/in · 16 help · 17 about · 18 Office in a Box · 19 site survey · 20 all categories · 21 legal · 22 basket page
- `screenshots/admin/` — 01–32, one per module plus order / quote / product drawers, bulk-upload preview and the three Staff and roles tabs

## Content sources (`content-sources/`)
The documents the copy was written from: *Website Content & Sales Master v1.1*, *Industry Solutions & Sales Packaging v1.4*, *Solution Definition Register v1.5* (plain text), brand identity v2.1, brand architecture and products/services by division. If README copy and a design file disagree, **the design file wins**; these are background for content rules, not a second copy source.

## Before launch — owner inputs still needed (keep placeholders until supplied)
Phone, email, WhatsApp number, social links · delivery zones fees and times · legal text (terms, privacy, cookies) · business-account review time · real product photography, kit room photos and D’Source hero photos (all `<image-slot>` captions say what goes there) · delivered case studies (D’Matek Work) · Damilola portrait and story · IléMesh subdomain · confirm rights for every photo in `assets/photos/` · confirm D’Foundry “Yours to own” wording and “within two working days” reply promise · payment provider accounts (Paystack/Flutterwave).

## Files in this bundle
```
Dmatek Website v3.4.dc.html   D’Matek site (Part 1)
DFoundry Hero.dc.html         D’Foundry site (Part 2) — now with Adire footer strip
Droplet Case Study.dc.html    D’Foundry case study
DSource v7.4.dc.html          D’Source storefront (Part 3)
DSource Admin v2.dc.html      D’Source back office (Part 4)
DFOUNDRY_PORT.md              exact-replication guide for D’Foundry
DSOURCE_PORT.md               exact-replication guide for D’Source storefront + admin
dfoundry-source/              D’Foundry + Droplet split into template + logic
dsource-source/               storefront-* and admin-* split into template + logic
CLAUDE_CODE_PROMPT.txt        prompt to paste into Claude Code
screenshots/                  static visual references (see above)
content-sources/              source content documents
support.js, image-slot.js     prototype runtime — reference only, do not port
assets/                       logos, marks, D’Foundry/Droplet images, assets/photos/ (D’Matek photography)
README.md                     this document
```
Cross-file links inside the bundle point at each other (D’Matek → D’Source v7.4, D’Foundry → D’Matek v3.4).
