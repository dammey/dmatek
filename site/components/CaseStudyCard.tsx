  ...
  return (
    ...
    >
      ...
      <div className="grid items-center gap-6 sm:grid-cols-2 sm:gap-11">
        <div
          className="relative mx-auto flex aspect-square w-full max-w-[320px] flex-col items-center justify-center overflow-hidden p-[clamp(24px,3vw,40px)] text-center text-cream"
          style={{
            background: study.photoSrc ? undefined : "radial-gradient(circle at 30% 26%, #0B4B3D, #043028)",
            borderRadius: "50% 50% 42% 58% / 46% 50% 50% 54%",
            animation: "dm-drift 20s ease-in-out infinite",
          }}
          role={study.photoSrc ? undefined : "img"}
          aria-label={study.photoSrc ? undefined : "Placeholder installation photograph"}
        >
          {study.photoSrc && (
            <>
              <Image src={study.photoSrc} alt={study.photo} fill sizes="320px" className="object-cover" />
              <div
                className="absolute inset-0"
                style={{ background: "linear-gradient(180deg,rgba(6,56,46,0.15),rgba(4,48,40,0.72))" }}
              />
              <p className="relative m-0 max-w-[16em] text-[15px] leading-[1.6] text-cream/86">{study.photo}</p>
            </>
          )}
          {!study.photoSrc && (
            <>
        ...
        <blockquote className="m-0 rounded-[clamp(22px,3vw,36px)] bg-cream p-[clamp(26px,3vw,42px)] shadow-[0_14px_36px_rgba(6,56,46,0.08)]">
          ...
          {study.attrib && <footer className="mt-5 text-[13px] font-bold tracking-[0.06em] text-progress">{study.attrib}</footer>}
        </blockquote>
      </div>
    </Reveal>
  );
